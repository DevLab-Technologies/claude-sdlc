...
flowchart TD
    ...
    ARCH --> IMPL[implementers<br/>one per task, parallel where declared]
...
| `sdlc-product-owner` | PRD, stories, acceptance criteria, priority | Say how to build it |
...
| `sdlc-architect` | Architecture, interface contracts, ADRs, workplan, test strategy | Write feature code |
...
| `sdlc-qa-functional` | Test plan, execution, defect reports, fix verification | Pass on inspection alone |
...
- `.sdlc/features/<slug>/10-investigations/` — root-cause investigations
...
- Add a security phase by inserting `security-auditor` into phase 7 alongside code review.
